...
export default async function Page({params}:{params:Promise<{slug:string}>}){
  ...
  return <>
    <section className={`detail-hero ${t.brand.toLowerCase()}`}><div className="container">
      ...
      <div className="detail-grid"><div><p className="eyebrow">{t.brand}</p><h1>{t.name}</h1><p>{t.description}</p><div className="detail-badges"><span><Gauge/>{t.hp}</span><span><TractorIcon/>{t.drive}</span><span><Cog/>{t.lift} lift</span></div></div><div className="detail-image"><img src={t.image} alt={t.name}/></div></div>
    </div></section>
